COPPER PRICE ETL PIPELINE (PYTHON + SQLALCHEMY + MYSQL)
=======================================================

A production-grade ETL pipeline that extracts global copper prices from
TradingEconomics, transforms the data using pandas, and loads it into a
MySQL database using SQLAlchemy with UPSERT logic. The pipeline is
idempotent, modular, and follows industry-standard data engineering
practices.
...
• Regex-based extraction from meta description
...
├── etl_log.txt           # Runtime logs (auto-generated)
└── output/               # CSV outputs
...
------------------------------------------------------
1. EXTRACT
   - Scrapes copper price from TradingEconomics
   - Extracts price, unit, currency, reported date
   - Parses meta description using regex
...
3. Check outputs:
   - output/copper_price.csv
   - etl_log.txt
   - MySQL table: copper_price
...
5. What challenges did you solve?
   - Regex extraction from meta description
